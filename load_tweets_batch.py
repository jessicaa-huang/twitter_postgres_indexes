...
#    sql = sqlalchemy.sql.text('''
   ...
   # ;
 #   ''')
 #   res = connection.execute(sql,{'urls':url}).first()
#  if res is None:
#        sql = sqlalchemy.sql.text('''
   #     select id_urls 
   #     from urls
...
def _insert_tweets(connection,input_tweets):
    ...
    for tweet in input_tweets:
        ...
        ########################################
        #user_id_urls = None 
        #if 'user' in tweet and 'url' in tweet['user']:
        #    user_id_urls = tweet['user']['url']
        #else:
        #    user_id_urls = get_id_urls(tweet['user']['url'])
        #print("This is the user_id_url: ", user_id_urls)
        try:
            user_id_urls = tweet['user']['url']
        except Exception as e:
            print("Error accessing tweet['user']['url']: ", e)
...
#            print("This is the url", url)
            tweet_urls.append({
                'id_tweets':tweet['id'],
                'urls': url.get('expanded_url', None),
                #'id_urls':id_urls,
                })
